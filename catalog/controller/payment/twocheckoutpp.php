...
class ControllerPaymentTwoCheckoutpp extends Controller {
	protected function index() {
		...
		$this->data['sid'] = $this->config->get('twocheckoutpp_account');
		...
		$this->data['return_url'] = $this->url->link('payment/twocheckoutpp/callback', '', 'SSL');
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/twocheckoutpp.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/twocheckoutpp.tpl';
		} else {
			$this->template = 'default/template/payment/twocheckoutpp.tpl';
		}	
	...
	public function callback() {
		...
		if (strtoupper(md5($this->config->get('twocheckoutpp_secret') . $this->config->get('twocheckoutpp_account') . $order_number . $this->request->request['total'])) == $this->request->request['key']) {
			if ($this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false) == $this->request->request['total']) {
				$this->model_checkout_order->confirm($this->request->request['cart_order_id'], $this->config->get('twocheckoutpp_order_status_id'));
			} else {
